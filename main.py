...
def main():
    today_titles, urls = get_news_titles(news_source)
    if os.path.isfile("./" + file_name):
        new_titles = compare_news_titles(today_titles,file_name)
        for title in new_titles:
            print(title)
        for url in urls:
            webbrowser.open(url)
    save_news_titles_to_file(today_titles)
...
def get_news_titles(url):
    ...
    # TODO: change the below attribute(s) depending on news website
